...
from openapi_server.models.inline_response200_links import InlineResponse200Links  # noqa: E501
from openapi_server.models.inline_response200_data import InlineResponse200Data, InlineResponse200Message, InlineResponse200Links1  # noqa: E501
...
def get_message_username(username, token_info):  # noqa: E501
    ...
    return 'Redirect', 308, {'Location': f"/message/{username}/from/0"}
...
def get_message_username_from_messageid(username, messageid, token_info=None):  # noqa: E501
    ...
    base_links = {
            "self": f"/message/{user}/from/{messageid}",
            "prev": f"/message/{user}/from/{prev_messageid}",
            "next": f"/message/{user}/from/{messageid}"

    }
    if more_than_10:
        base_links["next"] = f"/message/{user}/from/{int(messageid) + 10}"
...
def get_message_username_messageid(username, messageid, token_info = None):  # noqa: E501
    ...
    print(f"{user} richiede l'accesso al messagio {messageid}")
...
def post_allmessage(token_info=None):  # noqa: E501
    ...
    base_links = {
            "self": f"/message/{user}/from/0",
            "prev": f"/message/{user}/from/0",
            "next": f"/message/{user}/from/0"
    }
    if more_than_10:
        base_links["next"] = f"/message/{user}/from/10"
    
    links = InlineResponse200Links(base_links["self"], base_links["next"], base_links["prev"])  

    return InlineResponse200(data = InlineResponse200Data(messages), links= links)

def post_createmessage(token_info=None, inline_object=None):  # noqa: E501
    """Invia messaggio
    ...
    if connexion.request.is_json:
        inline_object = InlineObject.from_dict(connexion.request.get_json()).to_dict()  # noqa: E501
    ...
    id_rcv = 0
    for m in db_message:
        if m["receiver"] == inline_object["data"]["receiver"]:
            id_rcv += 1
    ...
    msg = {
        "receiver": inline_object["data"]["receiver"],
        "from": get_username_from_uid(token_info["uid"]),
        "data": {
            "offer": OfferMessage(inline_object["data"]["offer"]["code"], inline_object["data"]["offer"]["description"] ), 
            "date": datetime.date.today(), 
            "id_snd": id_snd, 
            "id_rcv": id_rcv
        }, 
        "link": {
            "self_snd": f"/{get_username_from_uid(token_info['uid'])}/{id_snd}", 
            "prev_snd": f"/{get_username_from_uid(token_info['uid'])}/{prev_snd}", 
            "next_snd": f"/{get_username_from_uid(token_info['uid'])}/{id_snd}", 
            "self_rcv": f"/{inline_object['data']['receiver']}/{id_rcv}", 
            "prev_rcv": f"/{inline_object['data']['receiver']}/{prev_rcv}", 
            "next_rcv": f"/{inline_object['data']['receiver']}/{id_rcv}"
        }
    }
...
def post_login(maps_v1_credentials=None):  # noqa: E501
    ...
    if len(maps_v1_credentials.username) > 0 and len(maps_v1_credentials.password) > 0: #TODO si può controllare che pwd esista in un db
        #TODO non posso fare questo controllo perché non esiste un endpoint per registrarsi
        # for user in db_registered_users:
        #     if maps_v1_credentials.username == user:
                user = maps_v1_credentials.username #TODO poi cancella questa roba
    ...
    #TODO manca endpoint per registrarsi